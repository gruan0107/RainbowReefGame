...
abstract public class Thing
{
    ...
    public boolean isOverlap(Thing thing, int x, int y) {
        ...
        for (int offset = 0; offset < this.getWidth()/2 ;offset += this.getHeight()/2) {
            double distance = Math.sqrt(Math.pow((double)(this.x + offset - x),2.0) + Math.pow((double)(this.y - y),2.0));
            if (distance < radius1 + radius2) {
                return true;
            }
            distance = Math.sqrt(Math.pow((double)(this.x - offset - x),2.0) + Math.pow((double)(this.y - y),2.0));
            if (distance < radius1 + radius2) {
                return true;
            }
        }
